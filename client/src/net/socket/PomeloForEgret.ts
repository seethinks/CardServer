...
class PomeloForEgret
{
    ...
    public init(params,cb:Function,serverType:string):void{
    ...
    public initWebSocket(host,port,cb:Function):void{
    ...
    private heartbeatTimeoutCb(data) {
        ...
        if(gap > this.gapThreshold) {
            ...
            this.emit(PomeloForEgret.EVENT_HEART_BEAT_TIMEOUT,data);
            this._disconnect();
        }
    }
    public off(event?,fn?){
    ...
    private handshake(resData){
        ...
        if(data.code === this.RES_OLD_CLIENT) {
            this.emit(PomeloForEgret.EVENT_IO_ERROR, 'client version not fullfill');
            return;
        }

        if(data.code !== this.RES_OK) {
            this.emit(PomeloForEgret.EVENT_IO_ERROR, 'handshake fail');
            return;
        }
...
//				}
        ...
        //{
        //    route = PomeloForEgret.requests[id].route;
        //}
        //
